...
class ImagineReactor(MessageReactor):
    def handle_message(self, instance, event_type: str, message: InterpretedMessage) -> bool:
        if not message.has_kind(MessageKind.GENERIC_SUCCESS):
            return False
        prompt = message.prompt
        if not prompt or not message.attachments:
            return False

        job = self.find_and_finish_image_job(instance, JobAction.IMAGINE, prompt, message)
        if job is not None:
            instance.schedule_image_indexing(job.action, message.raw, job)
            self._safe_dump(instance, "dump_imagine", message=message.raw, prompt=prompt)
        return job is not None


__all__ = ["ImagineReactor"]
