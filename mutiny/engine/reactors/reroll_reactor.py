...
class RerollReactor(MessageReactor):
    def handle_message(self, instance, event_type: str, message: InterpretedMessage) -> bool:
        ...
        job = self.find_and_finish_image_job(instance, JobAction.REROLL, prompt, message)
        if job is not None:
            instance.schedule_image_indexing(job.action, message.raw, job)
            self._safe_dump(instance, "dump_reroll", message=message.raw, prompt=prompt)
        return job is not None


__all__ = ["RerollReactor"]
